...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    # Only copy if input modification is needed, otherwise keep same reference
    bedrock_tool_name = input_tool_name
    if not bedrock_tool_name[0].isalpha():
        bedrock_tool_name = "a" + bedrock_tool_name

    # Use list comprehension for best performance (avoid repeated str concatenation)
    # Avoid local function call overhead: inline the logic
    valid_chars = [
        c if (c.isalnum() or c == "_") else "_"
        for c in bedrock_tool_name
    ]
