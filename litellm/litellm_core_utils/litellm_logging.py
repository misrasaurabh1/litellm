...
class Logging(LiteLLMLoggingBaseClass):
    ...
    def __init__(
        ...
        # Convert messages to unified format with minimal branching
        _input = messages
        ...
        self.litellm_trace_id = litellm_trace_id or str(uuid.uuid4())
        self.function_id = function_id
        self.streaming_chunks = []
        self.sync_streaming_chunks = []
        self.log_raw_request_response = log_raw_request_response

        # Assign dynamic callback lists
        self.dynamic_input_callbacks = dynamic_input_callbacks
        self.dynamic_success_callbacks = dynamic_success_callbacks
        self.dynamic_async_success_callbacks = dynamic_async_success_callbacks
        self.dynamic_failure_callbacks = dynamic_failure_callbacks
        self.dynamic_async_failure_callbacks = dynamic_async_failure_callbacks

        # Process dynamic callbacks (likely in-place optimization in canonical implementation)
        self.process_dynamic_callbacks()

        # Standard * callback keys/params
        self.standard_callback_dynamic_params = self.initialize_standard_callback_dynamic_params(kwargs)
        self.standard_built_in_tools_params = self.initialize_standard_built_in_tools_params(kwargs)
    ...
    def get_custom_logger_for_prompt_management(
        ...
        if litellm.vector_store_registry is not None:
            if vector_store_to_run := litellm.vector_store_registry.get_vector_store_to_run(
                ...
                vector_store_custom_logger = litellm.ProviderConfigManager.get_provider_vector_store_config(
                    provider=cast(
    ...
    def post_call(self, original_response, input=None, api_key=None, additional_args={}):
        ...
        # Avoid unnecessary JSON serialization unless non-dict type is required
        if isinstance(original_response, dict):
            orig_resp_to_log = json.dumps(original_response)
        else:
            orig_resp_to_log = original_response

        try:
            mcd = self.model_call_details
            mcd["input"] = input
            mcd["api_key"] = api_key
            mcd["original_response"] = orig_resp_to_log
            mcd["additional_args"] = additional_args
            mcd["log_event_type"] = "post_api_call"

            log_data = mcd.get("original_response", mcd)
            if json_logs:
                verbose_logger.debug(f"RAW RESPONSE:\n{log_data}\n\n")
            else:
                print_verbose(f"RAW RESPONSE:\n{log_data}\n\n")

            # If logger_fn present, call it in try/except for non-blocking behavior
            logger_fn = getattr(self, "logger_fn", None)
            ...
            # Redact response as needed (in-place redaction)
            original_response = redact_message_input_output_from_logging(
                model_call_details=mcd, result=original_response
            )
            # Input Integration Logging -> call each callback, skip cost where lists are empty
            callbacks = litellm.input_callback
            dyn_cbs = self.dynamic_input_callbacks
            if dyn_cbs:
                callbacks = callbacks + dyn_cbs
            for callback in callbacks:
                try:
                    if callback == "sentry" and add_breadcrumb:
                        verbose_logger.debug("reaches sentry breadcrumbing")
                        try:
                            details_to_log = copy.deepcopy(mcd)
                        except Exception:
                            details_to_log = mcd
                        if litellm.turn_off_message_logging:
                            details_to_log.pop("messages", None)
                    ...
                    verbose_logger.exception(
                        f"LiteLLM.LoggingError: [Non-Blocking] Exception occurred while post-call logging with integrations {str(e)}"
                    )
                    verbose_logger.debug(
                        f"LiteLLM.Logging: is sentry capture exception initialized {capture_exception}"
                    )
            ...
            verbose_logger.exception(f"LiteLLM.LoggingError: [Non-Blocking] Exception occurred while logging {str(e)}")
    ...
    def should_run_callback(self, callback: litellm.CALLBACK_TYPES, litellm_params: dict, event_hook: str) -> bool:
        ...
        if EnterpriseCallbackControls is not None and EnterpriseCallbackControls.is_callback_disabled_via_headers(
        ...
        try:
                ...
                if self._is_recognized_call_type_for_logging(logging_result=logging_result):
                    ...
                    if hidden_params:
                        ...
                        if self.model_call_details.get("litellm_params") is not None:
                            ...
                            self.model_call_details["litellm_params"]["metadata"]["hidden_params"] = getattr(  # type: ignore
            ...
            if isinstance(result, ResponsesAPIResponse):
                ...
                setattr(
                    result,
                    "usage",
                    ResponseAPILoggingUtils._transform_response_api_usage_to_chat_usage(result.usage),
                )
        ...
        try:
            ...
            for callback in callbacks:
                try:
                    ...
                    if (
                        callback == "openmeter"
                        and self.model_call_details.get("litellm_params", {}).get("acompletion", False) is not True
                        and self.model_call_details.get("litellm_params", {}).get("aembedding", False) is not True
                        and self.model_call_details.get("litellm_params", {}).get("aimage_generation", False)
                        is not True
                        and self.model_call_details.get("litellm_params", {}).get("atranscription", False) is not True
                    ):
    ...
    return {
        k: (
            ...
            # Apply masking to sensitive keys
                ...
                (v[: unmasked_length // 2] + "*" * number_of_asterisks + v[-unmasked_length // 2 :])
                if (isinstance(v, str) and len(v) > unmasked_length and number_of_asterisks is not None)
                else (
                    (v[: unmasked_length // 2] + "*" * (len(v) - unmasked_length) + v[-unmasked_length // 2 :])
...
def _init_custom_logger_compatible_class(  # noqa: PLR0915
    ...
    try:
        ...
        if logging_integration == "agentops":  # Add AgentOps initialization
            ...
            for callback in _in_memory_loggers:
                if isinstance(callback, OpenTelemetry) and callback.callback_name == "arize_phoenix":
                    return callback  # type: ignore
            ...
            for callback in _in_memory_loggers:
                if isinstance(callback, OpenTelemetry) and callback.callback_name == "langfuse_otel":
                    return callback  # type: ignore
            _otel_logger = OpenTelemetry(config=otel_config, callback_name="langfuse_otel")
            _in_memory_loggers.append(_otel_logger)
            return _otel_logger  # type: ignore
...
class StandardLoggingPayloadSetup:
        ...
        for key in StandardLoggingAdditionalHeaders.__annotations__.keys():
            ...
            if _key in additiona_headers:
                try:
                    ...
                    verbose_logger.debug(f"Could not convert {additiona_headers[_key]} to int for key {key}.")
    ...
    def _get_request_tags(metadata: dict, proxy_server_request: dict) -> List[str]:
        request_tags = metadata.get("tags", []) if isinstance(metadata.get("tags", []), list) else []
...
def get_standard_logging_object_payload(
    ...
    try:
        ...
        metadata: dict = litellm_params.get("litellm_metadata") or litellm_params.get("metadata", None) or {}
...
def get_standard_logging_metadata(
    ...
    if isinstance(metadata, dict):
        ...
        clean_metadata = StandardLoggingMetadata(
            **{  # type: ignore
                key: metadata[key] for key in StandardLoggingMetadata.__annotations__.keys() if key in metadata
            }
        )
