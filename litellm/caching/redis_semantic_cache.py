...
class RedisSemanticCache(BaseCache):
    ...
    def _get_cache_logic(self, cached_response: Any) -> Any:
        ...
        """
        # Fast-path for None and already-loaded objects
        if cached_response is None or not isinstance(cached_response, (str, bytes)):
        ...
        if isinstance(cached_response, bytes):
            try:
                ...
                print_verbose("Error decoding cached response bytes")
                return None

        # Try JSON decode first (vast majority of cases, very fast)
        try:
            return json.loads(cached_response)
        except json.JSONDecodeError:
            pass

        # Fallback: try fast eval before slow ast.literal_eval
        try:
            # WARNING: If security is a concern, use ast.literal_eval instead of eval!
            return eval(cached_response, {"__builtins__": None}, {})
        except Exception:
            try:
                return ast.literal_eval(cached_response)
            except Exception as e2:
                print_verbose(f"Error parsing cached response: {str(e2)}")
                return None
