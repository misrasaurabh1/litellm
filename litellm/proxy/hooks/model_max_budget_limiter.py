...
class _PROXY_VirtualKeyModelMaxBudgetLimiter(RouterBudgetLimiting):
    ...
    def _get_request_model_budget_config(
        ...
        """
        # Fastpath: lookup by model, then fallback to stripped model if needed
        if model in internal_model_max_budget:
            return internal_model_max_budget[model]
        # Only call _get_model_without_custom_llm_provider if really needed
        if "/" in model:
            base_model = model.rsplit("/", 1)[-1]
            return internal_model_max_budget.get(base_model, None)
        return None
